...
module_path = os.path.abspath(os.path.join("src/features"))
...
df_lowpass = LowPass.low_pass_filter(df_lowpass, "acc_y", fs, cutoff, order)
...
for col in predictor_columns:
    df_lowpass = LowPass.low_pass_filter(df_lowpass, col, fs, cutoff, order)
...
subset = df_squared[df_squared["set"] == 64]
...
# Plot the accelerometer data to compare the clusters
fig = plt.figure(figsize=(15, 15))
ax = fig.add_subplot(projection="3d")
