...
module_path = os.path.abspath(os.path.join("src/models"))
...
df_train = df.drop(["category", "set", "participant"], axis=1)
...
fig, ax = plt.subplots(figsize=(10, 5))
df_train["label"].value_counts().plot(kind="bar", ax=ax, color="yellow", label="Total")
y_train.value_counts().plot(kind="bar", ax=ax, color="navy", label="Train")
y_test.value_counts().plot(kind="bar", ax=ax, color="white", label="Test")
plt.legend()
plt.show()
